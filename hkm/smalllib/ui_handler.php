...
if (!class_exists('ui_handler')):
    class ui_handler
    {
        ...
        public static function ui_select_creation_complete($series, $id = null, $name, $default = "select one", $current_val)
        {
            /* $nid = $id == null ? '' : 'id="' . $id . '"';
             $ui = '<select ' . $nid . ' name="' . $name . '">';
             $ui .= '<option value="">' . $default . '</option>';
             foreach ($series as $k => $v) {
                 $content = $v;
                 $ui .= '<option value="' . $k . '" s>' . $content . '</option>';
             }
             $ui .= '</select>';*/

            $ui = '<select class="of-input" name="' . $name . '" id="' . $id . '">';
            $ui .= '<option value="">' . $default . '</option>';
            foreach ($series as $k => $v) {
                $selected = intval($current_val) == intval($k) ? ' selected="selected"' : '';
                /*   if( $current_val != '' ) {
                       if ( $current_val == $k) { $selected = ' selected="selected"';}
                   }*/
                $ui .= '<option' . $selected . ' value="' . esc_attr($k) . '">' . esc_html($v) . '</option>';
            }
            $ui .= '</select>';
