...
class Response_Metabox
{
    ...
    function end()
    {
        ...
        new Eclipse_RW_MetaBox_Taxonomy($final);
    }
 ...
 * Meta Box class
 */
if (!class_exists('Eclipse_RW_MetaBox')) {
    class Eclipse_RW_MetaBox
    {
        ...
        ';
            foreach ($ids as $id) {
                echo "
            jQuery('#picker-$id').farbtastic('#$id');
            jQuery('#select-$id').click(function(){
                jQuery('#picker-$id').toggle();
                return false;
            });
            ";
            }
        ...
        function show()
        {
            ...
            wp_nonce_field(basename(__FILE__), 'Eclipse_RW_MetaBox_nonce');
        ...
        {
            $this->show_field_begin($field, $meta);
            $tmp = "<input type='checkbox' class='checkbox";
            if ($this->Endswith($field['id'], "_toggle")) {
                $tmp .= " toggle'";
            } else {
                $tmp .= "'";
            }
            $tmp .= " name='{$field['id']}' id='checkbox-{$field['id']}' " . (($meta === 'on' || $meta === '1') ? 'checked="checked"' : '') . " value='1'/> {$field['desc']}</td>";
            echo $tmp;
        }
            ...
            <script type="text/javascript">
                jQuery(function ($) {
                    function update(base) {
                        ...
                        jQuery('.right_list .action').show();
                        jQuery('.left_list .action').hide();
                    }


                    jQuery(".left_list .list_items").delegate(".action", "click", function () {
                    ...
                    jQuery(".right_list .list_items").delegate(".action", "click", function () {
                        var item = jQuery(this).closest('.list_item');
                        jQuery(this).val('Add');
                    ...
                    jQuery(".right_list .list_items").sortable({
                        update: function () {
                            update(jQuery(this).closest(".section_order"));
                        },
                    ...
                    jQuery(".left_list .list_items").sortable({
        ...
        function save($post_id)
        {
            ...
            if ('admin-ajax.php' != $pagenow) {
                ...
                if ((defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) // check autosave
                    ...
                    || (!check_admin_referer(basename(__FILE__), 'Eclipse_RW_MetaBox_nonce')) // verify nonce
                ...
                foreach ($this->tabs as $tab) {
                    foreach ($tab['fields'] as $field) {
                        ...
                        // validate meta value
                        if (class_exists('Eclipse_RW_MetaBox_Validate') && method_exists('Eclipse_RW_MetaBox_Validate', $field['validate_func'])) {
                            $new = call_user_func(array('Eclipse_RW_MetaBox_Validate', $field['validate_func']), $new);
                        }
...
/**
 * Extend Eclipse_RW_MetaBox class
 * Add field type: 'taxonomy'
 */
class Eclipse_RW_MetaBox_Taxonomy extends Eclipse_RW_MetaBox
{
...
function metabox_enqueue()
{
   /* $path = get_template_directory_uri() . "/core/library/js/";
    ...
    wp_register_style('metabox-tabs-css', HKM_LIBCSS . 'admin/metabox-tabs.css');
    wp_register_script('jf-metabox-tabs', LIBJS_ADMIN_MODEL . 'metabox-tabs.js');
    wp_enqueue_script('jf-metabox-tabs');
    wp_enqueue_script('jf-metabox-tabs');
    wp_enqueue_script('jquerycustom', LIBJS_ADMIN_MODEL . 'jquery-custom.js', array('jquery'));
