...
class ClassyOptions {
	...
	function admin_menu() {
		...
		//$page = add_theme_page( $this->name, $this->name, 'edit_theme_options', $this->id, array( $this, 'render' ) );
		$page = add_theme_page( $this->name, $this->name, 'edit_pages', $this->id, array( $this, 'render' ) );
	...
	function load_styles() {
		wp_enqueue_style('admin-style', CLASSY_OPTIONS_FRAMEWORK_CSS.'admin-style.css');
		wp_enqueue_style('color-picker', CLASSY_OPTIONS_FRAMEWORK_CSS.'colorpicker.css');
	...
	function load_scripts() {
		...
		wp_enqueue_script('color-picker', CLASSY_OPTIONS_FRAMEWORK_JS.'colorpicker.js', array('jquery'));
		wp_enqueue_script('options-custom', CLASSY_OPTIONS_FRAMEWORK_JS.'options-custom.js', array('jquery'));
		wp_enqueue_script('theme-options-custom', get_template_directory_uri().'/js/theme-options-custom.js', array('jquery'));
...
if ($options->get($ec_themeslug.'_custom_logo') == '1' && $logo['url'] != '') { ?>
	<h2 style="height:0px;top:0px;position:relative;">
		<img src="<?php echo stripslashes($logo['url']); ?>" alt="logo">
	</h2> <?php
	}
						
	else{ ?>
		<h2 class="sitename"><?php bloginfo('name'); ?></h2>
		<?php
	}	
?></div>
<div class="clear"></div>
	...
	<input type="submit" class="button-primary" name="update" value="<?php esc_attr_e( 'Save Options' ); ?>" />
		...
		<div id="top"><a href='#TOP'><img src="<?php echo CLASSY_OPTIONS_FRAMEWORK_IMG ;?>top.png" /></a></div>
