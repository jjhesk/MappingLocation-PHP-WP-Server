...
jQuery(function ($) {
    ...
    ReportContentEditor.prototype = {
        ...
        getCurrentReportHTML: function () {
            ...
            return minify(bank, d.minifyOptions());
        },
        ...
        loadReports: function (html) {
            if (html != '') {
                var d = this,
                //t = new String(html).replace(/^\s+|\s+$/gm, ''),
                    template_html = $.parseHTML(html);
                console.log(html);
                $.each(template_html, function (i, node) {
                    var a = $(node);
                  //  console.log(a);
                  //  console.log(i);
                    //t = new String(a.html()).replace(/^\s+|\s+$/gm, ''), empty = t == '';
                    //if (!empty)
                    d.addTemplate(i, a);
                });
            }
        ...
        toggle_frame: function () {
            var d = this, b = $(".rsContainer .subpage", d.$container);
            d.view_guide = !d.view_guide;
            $.each(b, function (e, f) {
                var h = $(this);
                if (d.view_guide) {
                    h.addClass("visible");
