...
//$ec_themenamefull.
$options = new ClassyOptions($ec_themename, "Linker Options");
...
$options
    ->section("Welcome")
        ->info("<h1>Eclipse for Linker Hong Kong</h1>
        
<p><strong>Linker Eclipse Professional Responsive WordPress Theme</strong></p>
...
")
    ->section("Design")
        ...
        ->subsection("Default Images")
            //->checkbox($ec_themeslug."_custom_logo220", "Default Bar Image thumb 220x100")
            ->upload($ec_themeslug."_logo220", "Bar Browse Logo 220x100")
            ->upload($ec_themeslug."_logo250", "Content Bar Feature 250x250")
            ->upload($ec_themeslug."_logo7901", "Content Bar Slider-One 790x500")
            ->upload($ec_themeslug."_logo7902", "Content Bar Slider-Two 790x250")
            ->subsection_end()
            ->open_outersection()
        ...
        ->section("Header")
        ->open_outersection()
            ->section_order("header_section_order", "Drag & Drop Elements", array('options' => 
            array(
            //design 1
            ...
            //design 3
             "response_logo_for_linker"=>"Logo only + download apps",
             "response_mobileapps"=>"download apps"
             ), 
             
             'default' => 'response_logo_menu'))
        ->close_outersection()
        ...
        ->subsection_end()
        ->subsection("API")
        ->info("A new key can be generated at <a href='http://code.google.com/apis/maps/documentation/javascript/v2/introduction.html#Obtaining_Key.'>ENTER</a>")
        ->text($ec_themeslug."_googleapi", "Google API Key")
        ->text($ec_themeslug."_bingmapapi", "Bing Map API Key")
        ->checkbox($ec_themeslug."_bingmapreal", "Bing Map Display in Real Photo")
        //
        ->subsection_end()
        ->subsection("Social")
            ...
            ->checkbox($ec_themeslug."_hide_rss_icon", "Hide RSS Icon"  )
        ->subsection_end()
        ->subsection("Tracking")
        ...
        ->section("Blog - Single")
        ->open_outersection()
            ->section_order($ec_themeslug."_blog_section_order", "Drag & Drop Elements", array('options' => 
            array(
            "response_post" => "Post Page", 
            "response_blog_slider" => "Feature Slider", 
            "response_twitterbar_section" => "Twitter Bar", 
            "response_portfolio_element" => "Portfolio",
            "response_recent_posts_element" => "Recent Posts",
            "response_bingmapapi_element"=>"bing map bar locations",
            "response_photowall_element"=>"foto wall",
            "response_navtop"=>"top menu nav bar",
            "response_navarhive_result"=>"nav menu bar for result arhive",
            ), "default" => 'response_blog_slider,response_portfolio_element,response_twitterbar_section,response_post'))
        ->close_outersection()
        ->subsection("Coupon Feature Settings")
            ->checkbox($ec_themeslug."_couponflip_toggle", "Flip on Coupon Single",  array('default' => false))
        ->subsection_end()
    ...
    ->section("Footer")
        ->open_outersection()
            ->checkbox($ec_themeslug."_disable_footer_toggle", "Footer", array('default' => true))
            ->text($ec_themeslug."_credit", "Copyright statement")
        ->close_outersection()
    ->section("Mobile Apps")
        ->open_outersection()
            ->text($ec_themeslug."_android_app_url", "URL to download the andriod application")
            ->text($ec_themeslug."_ios_app_url", "URL to download the IOS application")
        ->close_outersection()
;
}
